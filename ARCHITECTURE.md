...
away from it. The pivot happened. The shipping build is `NATIVE=1`:

- One generation is one whole frame: launch, slave compose in three
  bands, close, ship (blit inside the next FM window), flip. Frozen
  bands are structurally impossible; a late frame repeats the previous
  one whole.
