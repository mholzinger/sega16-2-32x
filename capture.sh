...
# Usage:
#   ./capture.sh                      # WHOLE recording (no time cap)
...
raw=false
start="00:00:00"
end=""      # empty = the whole recording (the old 2:50 default silently
            # truncated every long playthrough — Mike lost a boss-fight
            # capture window to it on 2026-08-30)
...
if [[ "$dedup_only" == false ]]; then
    ...
    echo "Extracting frames ($start → ${end:-end of recording})..."
    ...
    # ${arr[@]+...} guard: macOS bash 3.2 treats an empty array as unbound
    # under `set -u`, so a bare "${to_args[@]}" would abort the script.
    to_args=()
    [[ -n "$end" ]] && to_args=(-to "$end")
    ffmpeg -hide_banner -loglevel error -ss "$start" ${to_args[@]+"${to_args[@]}"} \
