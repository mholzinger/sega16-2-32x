...
def main():
    ...
    print(f'{"region":14s} {"per-frame":>10s} {"peak":>8s} {"total":>9s}')
    for n in names:
        vals = [int(x[n]) for x in rows]
        win = vals[a.a:a.b]
        print(f'{n:14s} {sum(win) / max(1, len(win)):10.1f} {max(win) if win else 0:8d} {sum(vals):9d}')
    sites = list(csv.DictReader(open(csvp + '.sites')))
    print('top writer PCs (arcade address = pc - 0x900000 when in the high copy):')
    for n in names:
