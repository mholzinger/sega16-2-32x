#!/usr/bin/env python3
"""SET-LIST PACKING ANALYSIS for the MD colour lines (LOOP29 272/274).

NOT A BAKER FOR THE SHIPPING LINE. The per-round colour-to-line bake
ALREADY SHIPS -- sh_src/pal_rounds_md.h, generated by
tools/bake_tilecram.py --emit-mds, consumed at m_main.c 2415-2429 under
MDSTATIC+MDROUND (LOOP29 192). Nothing includes this tool\'s header.

What this answers is the question that tool cannot: given a SET LIST,
does it partition into N CRAM lines of 15 pens? That is how LOOP29 274
found the real defect -- sets 22-36 are missing from the shipped round 2
and round 4 tables, which have only 4 and 5 pens spare -- and how 272
established that round 4 needs a split and round 2 does not.

Use it to size a rebuild before running it, on the UNION of the live
dumps and the arcade set lists (274: neither source is a superset).

    python3 tools/bake_mdlines.py [--lines 3] [--stats] [--out sh_src/mdlines_md.h]

The hardware constraint this measures: a tile draws from ONE MD CRAM line
