...
def run(fr):
    ...
    cmd=[ARES,"--frames",str(fr)]+(["--input",inp] if inp else [])+["--dump",f"sdram:{off:#x}:128:{cp}","--dump","sdram:0x28000:0x400:"+dp,"--dump","sdram:0x28F54:4:/tmp/bc_gens_%d.bin"%fr,rom]
    subprocess.run(cmd,check=True,capture_output=True)
    c=struct.unpack('>32I',open(cp,'rb').read()); d=struct.unpack('>256I',open(dp,'rb').read()); g=struct.unpack('>I',open('/tmp/bc_gens_%d.bin'%fr,'rb').read())[0]
    return c,d,g
prev=None
...
for fr in frames:
    ...
    if prev:
        ...
        dc=[c1[i]-c0[i] for i in range(32)]; dd=[d1[i]-d0[i] for i in range(256)]
        b=dc[18]; nb=dc[17]; t=dc[16]
        print(f"  f{pf}-{fr} ({fw} frames): batches {b} (non-empty {nb}) tiles {t} ({t/max(1,nb):.1f}/batch, {t/fw:.2f}/frame) | chunks {dc[19]} words {dc[20]} ({dc[20]/max(1,dc[19]):.0f}/chunk) | hits {dc[1]} claims {dc[2]} evict {dc[3]} | wiped {dc[10]} {dc[12]} {dc[14]} | gens {g1-g0}")
    prev=cur; pf=fr
