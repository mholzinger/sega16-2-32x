...
emu.register_frame_done(function()
    ...
    local tot = sh2:read_u32(0x06028000 + 8 * 4)
    local mskips = sh2:read_u32(0x06028000 + 7 * 4)
    local miss = sh2:read_u32(0x06028000 + 14 * 4)
    local swait = sh2:read_u32(0x06028000 + 4 * 4)
    local bdrain = sh2:read_u32(0x06028000 + 13 * 4)
    local amb = sh2:read_u32(0x06028000 + 15 * 4)
    local line = string.format('f=%d ent=%d skips=%d hv=%04x gpc=%08x', frames, ent, skips, hv, gpc)
    if n > 0 then
        line = line .. string.format(' compose=%.2fms mskips=%d miss=%.1f swait=%.2fms bdrain=%d', tot / n * 1.37e-3, mskips, miss / n, swait / n * 1.37e-3, bdrain) .. string.format(' amb=%d pp3=%d', amb, sh2:read_u32(0x06028000 + 16 * 4))
            .. string.format(' cp=%.2f cram=%.2f sw2=%.2f blit=%.2f flip=%.2f spr=%.2f',
                sh2:read_u32(0x06028000 + 0 * 4) / n * 1.37e-3,
                sh2:read_u32(0x06028000 + 2 * 4) / n * 1.37e-3,
                sh2:read_u32(0x06028000 + 3 * 4) / n * 1.37e-3,
                sh2:read_u32(0x06028000 + 5 * 4) / n * 1.37e-3,
                sh2:read_u32(0x06028000 + 6 * 4) / n * 1.37e-3,
                sh2:read_u32(0x06028000 + 12 * 4) / n * 1.37e-3)
    end
    out:write(line .. '\n')
    out:flush()
