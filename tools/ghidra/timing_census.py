                    ...
                    while j is not None and j.getAddress().getOffset() <= a.getOffset():
                        for rr in j.getReferencesFrom():
                            if not rr.getReferenceType().isFlow():
                                body.append('0x%X' % rr.getToAddress().getOffset())
                        j = j.getNext()
...
# ---- vectors ----
vectors = {}
for n, name in ((0x60, 'irq1'), (0x64, 'irq2'), (0x68, 'irq3'), (0x6C, 'irq4'), (0x70, 'irq5'), (0x74, 'irq6'), (0x78, 'irq7'), (0x04, 'reset_pc')):
    try:
