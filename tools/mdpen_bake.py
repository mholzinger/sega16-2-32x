...
def main():
    ...
    with open(a.out, 'w') as f:
        f.write('/* GENERATED by tools/mdpen_bake.py — do not edit.\n'
                ...
                ' * Plain const in .text: the .palscenes slot (0x1100 B) is full.\n'
                f' * Scenes: {", ".join(psb.LOADABLE)} (index = pscene id). */\n')
        f.write('#define MDSTATIC_N %d\n' % len(psb.LOADABLE))
        # scenes that inherit another scene's tables map to that table index,
        # so the runtime does not re-install (and re-walk) on a detect-only
