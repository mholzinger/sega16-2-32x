...
/* ===================== DIAG SLOT REGISTRY (2026-09-16) =====================
 * WHO OWNS EACH SLOT. Same role as the memory map at m_main.c:1523: the
 * authority, not grep. Two index collisions in two weeks cost two
 * debugging cycles each (LESSONS.md, "Counter indices collide").
 *
 * THE BLOCK IS 64 SLOTS AND IT IS FULL. 0x28000..0x280FF is 256 B; BM
 * starts at 0x28100. There is no slot 64. Do not add a counter by
 * picking a number -- take one of the FREE list below, or put it in
 * .bss the way mdalloc_ctr does.
 *
 * Generated by a preprocessor-aware census against the SH-2 flag half;
 * "LIVE" = the guard evaluates true on the line, "probe" = only under a
 * flag the line does not carry. A site's guard, not its comment,
 * decides. Verified against the TRUE line stamp (`make line`, not a
 * stale .build_flags): identical verdicts.
 *
 *   slot  owner (line)                                   state
 *   ----  -------------------------------------------    -----
 *    0    diag_add: compose+cap_drain span      14051    LIVE time
 *    1    diag_add: art-cache drain span        11756    LIVE time
 *    2    diag_add: apply_cram span             14318    LIVE time
 *    3    diag_add: slave tile wait span        14771    LIVE time
 *    4    diag_add: post-ack wait span          12162    LIVE time
 *    5    diag_add: window path end             12683    LIVE time  \ SEE NOTE A
 *         diag_add: nat_shipped ship span       14312    LIVE time  /
 *         DIAG[5]++ native commits               9033    probe (FLIP_DEFER)
 *    6    diag_add: flip+truth+restore span     12406    LIVE time
 *         DIAG[6]++ k2 arms                      8381    probe (FLIP_DEFER)
 *    7    missed flips = dropped frames         12238    LIVE count
 *    8    diag_add: WINDOW span                 15030    LIVE time
 *    9    cycles with >1 frame per vint     14386/196    LIVE count
 *   10    diag_add: band phase-0 span           11875    probe (!NATIVE_FRAME)
 *         bank already holds it                  7602    probe (BLIT_HASH)
 *   11    diag_add: MAPS DRAIN span             11747    LIVE time
 *         hash-verify mismatches                 7615    probe (BLIT_HASH_VERIFY)
 *   12    diag_add: band phase-2 span      11693/696    probe (!NATIVE_FRAME)
 *         hash-verify checks                     7616    probe (BLIT_HASH_VERIFY)
 *   13    win-two fallback                      11326    LIVE count
 *         queue-full deferrals                  15249    probe (!NATIVE_FRAME)
 *   14    art-cache miss telemetry               6502    LIVE count
 *   15    DISTINCT ambiguous sprite colours      4269    LIVE value
 *   16    sprite pp==3                           5848    LIVE count
 *   17    incomplete DREQ frames                13249    LIVE count
 *   18    BUILD_HASH32 (savestate self-id)      11411    LIVE value
 *   19    CRAM writes performed             1965/4461    probe (PAL_VBLANK/PAL_PEN)
 *         -- zeroed unconditionally at 11089, so it reads 0 on the line
 *   20    apply_cram groups skipped       4757..4624    LIVE count (one owner,
 *                                                        four sites)
 *   21    preempt-blit pickup timeouts    12649,14290    LIVE count (two paths)
 *   22    preempt-blit echo timeouts      12665,14301    LIVE count (two paths)
 *         slave never captured                   8305    LATENT -- see NOTE B
 *   23    diag_add: blit span                   12616    probe (WIN_SPLIT_PROBE)
 *   24    FM_TEST read-half PASS                 8018    LIVE count
 *         diag_add: blit span                   12668    probe (WIN_SPLIT_PROBE)
 *   25    FM_TEST read-half FAIL                 8020    LIVE count
 *         window line estimate            12628/12630    probe (WIN_SPLIT_PROBE)
 *   26    sprite-half tracer              12961/13001    LIVE count
 *   27    slave echo timeout               10129/10164    LIVE count
 *   28    blit windows                    12662,14291    LIVE count (two paths)
 *   29    held 68K releases                      8420    LIVE count
 *         ROW_DEFER deferred rows         7255..7093    probe (ROW_DEFER off:
 *                                                        NO_ROW_DEFER is on)
 ...
 *   35    *** COLLISION ***
 *         mdp_assign_set: set assigns       3059,3177    LIVE count
 *         bm_tail_body: wild sprite groups  4244,4257    LIVE count
 *   36    *** COLLISION, and the count is DESTROYED ***
 *         mdp_claim_pen: nearest-colour fallbacks 2738   LIVE count
 *         r60_pkt_flip parity: DIAG[36] ^= 1     15342   LIVE bit
 *         The XOR runs every gap, so neither reads. Known since
 *         STATE.md; this is the file:line pair.
 *   37    *** COLLISION ***
 *         mdp_free_set: set frees/invalidations   2660   LIVE count
 *         k2f_pendA arms                         16477   LIVE count
 *   38    *** COLLISION ***
 *         tolerated drifts                 16356,16424   LIVE count
 *         k2f_pendB arms                         16478   LIVE count
 *   39    *** COLLISION, FOUR OWNERS ***
 *         flip_span: B carried across swap        8633   LIVE count
 *         k2 path                                14469   LIVE count
 *         CAT1_MD                                15942   LIVE count
 *         HOT evict (vage < 12)                  15950   LIVE count
 *   40    builder: tile batches                  15408   LIVE count
 *   41    builder: NT chunks                     16196   LIVE count
 *         no heartbeat                     12216,12222   probe (SPAN_PROBE)
 *   42    *** COLLISION, FOUR OWNERS ***
 *         blit_half audit                         7656   LIVE count
 *         flip_span: A carried across swap        8616   LIVE count
 *         k2: A unconsumed / B unconsumed  14432,14471   LIVE count
 *   43    build deferred whole                   15347   LIVE count
 *         drain-wait span                        12821   probe (!FB_XPORT)
 *   44    k2 edge declines                        8369   LIVE count
 *   45    flip position from ISR entry, SUM       8478   LIVE value
 *   46    flip position from ISR entry, MAX       8479   LIVE value
 *   47    k2 seen too late to flip                9152   LIVE count
 *   48    max cut depth                          16212   LIVE value
 *   49    V-ISR entries                           8728   LIVE count
 *   50    *** COLLISION *** (already marked at 15967)
 *         DREQ words landed                      12894   LIVE sum
 *         MD allocator evictions                 15967   LIVE count
 *   51    *** COLLISION *** (already marked at 16376)
 *         DREQ K                                 12894   LIVE sum
 *         in-place recolours                     16376   LIVE count
 *   52    *** COLLISION ***
 *         cram_paint_spr: sprite sets painted     4684   LIVE count
 *         DREQ nrec                              12895   LIVE sum
 *   53    *** COLLISION *** (already marked at 15918)
 *         DREQ records                           12895   LIVE count
 *         md_tag claims                          15918   LIVE count
 *   54    pages actually copied                   6745   probe (TILE_RATE)  FREE
 *   55    cycles with any dirt                   14046   probe (TILE_RATE)  FREE
 *   56    body-fallback flip                     12391   LIVE count
 *         pg_pending popcount                    14047   probe (TILE_RATE)
 *   57    tiles sent                             15469   LIVE sum
 *   58    ISR flips                          9034,9212   LIVE count
 *   59    stale window                            8995   LIVE count
 *   60    k2 c0 == 0 early-out                    9111   LIVE count
 *   61    k1 family                  9066,9116,9144   LIVE count (one owner)
 *   62    flip dt MAX                             9216   LIVE value
 *   63    flip dt SUM                             9217   LIVE value
 *   64+   DOES NOT EXIST. The block ends at 0x280FF.
 *
 * FREE ON THE LINE (probe-flag owners only, so reusing one collides
 * ONLY in that probe build -- say so at the new site):
 *     10, 12, 19, 23, 32, 33, 54, 55
 * There is no slot that is free in every build. A NEW cumulative
 * counter belongs in .bss (see mdalloc_ctr), not here.
 *
 * NOTE A -- slot 5 has two live diag_add spans (12683 and 14312) that
 * measure different things, and 8802 reads slot 5 as "the SHIP". It is
 * NOT established that both execute under NATIVE_FRAME; if they do,
 * slot 5 is a sum of two spans and every ship-span number taken from it
 * is wrong. Check before quoting it.
 *
 * NOTE B -- slot 22 is safe only because this stamp has TEXTCAP_SLAVE
 * OFF. The Makefile defines TEXTCAP_SLAVE by default (it is suppressed
 * only by TEXTCAPMASTER=1), so an ordinary build turns 8305 on and slot
 * 22 becomes three owners.
 *
 * MDA / mdalloc_ctr[]: its own registry lives at the array
 * declaration (search `FULL SLOT REGISTRY`). Six of its slots were
 * double-booked until 2026-09-16.
 * ========================================================================= */
 ...
 *
 * FULL SLOT REGISTRY (2026-09-16 census, MDA(n)/MDA_ADD(n) macro forms
 * included -- a grep for `MDA[` or `MDA_ADD` alone MISSES `MDA(19)` and
 * that is how the 16..21 collision below stayed hidden). Every slot with
 * its writer's line:
 *  [0] cells reaching the allocator      15833   [1] key resident   15893
 *  [2] free-way claims  15919            [3] evictions      15968
 *  [4] cat-1 slot-pressure declines      15943
 *  [5] blanked: slot dirty, cut mode     16005
 *  [6] blanked: slot dirty, no cut       16020
 *  [7] blanked: FG cell empty            15807
 *  [8] blanked: bottom band              15773
 *  [9] mds_flush calls   3255            [10] tags wiped by it   3257
 * [11] mds_install calls 3238            [12] tags wiped by it   3245
 * [13] mdp_free_set calls 2646           [14] tags wiped by it   2544, 2657
 * [15] free_set declined: pinned by the scene table   2584
 * [16] mdp_assign_set                     3126
 * [17] held pens reclaimed                16327
 * [18] drift tolerated                    16417
 * [19] burned set claiming a pen          2693   \
 * [20] ... with no exclusive pen left     2694    > mdp_claim_pen:
 * [21] ... and a shareable match          2695   /  the PEN STARVATION trio
 * [22] pattern identical: keep            3173
 * [23] pattern differs: wipe              3174
 * [24] onscr accounting                   2554
 * [25] tags kept: same pattern            3058
 * [26] 3066   [27] extend kept the old pen 2786   [28] 2794
 * [29] held pens reclaimed                16287
 * [30] refused: not in the scene's table  2937
 * [31] FREE
 * [32] tiles sent      [33] non-empty batches   [34] batches      15471
 * [35] cell chunks     [36] their WORD COUNT                      16186
 * [37] art riding a chunk's tail                                  16134
 * [38]..[47] FREE
 *
 * *** 16..21 WERE DOUBLE-BOOKED until 2026-09-16. *** The NOTES 51 batch
 * census (db8d834) reused [16]..[21], which LOOP29 154 (eea4cf8) had
 * already given to mdp_assign_set, the held/drift counters and the
 * mdp_claim_pen pen-starvation trio. SIX slots, not the two STATE.md
 * recorded. The census now lives at [32]..[37]; tools/batch_census.py
 * reads the new indices. Any pen-starvation or batch figure quoted
 * before this date is the SUM of two unrelated subsystems.
 *
 * [20] is a WORD COUNT and [19] a CHUNK COUNT at the census sites too,
 * so `[20] > [19]` was never the subset violation it looked like -- the
 * collision was real, but that particular arithmetic was not the proof.
 *
 * PROBE ONLY. */
volatile uint32_t mdalloc_ctr[48];
...
#ifdef MD_ALLOC_WHY
                    MDA_ADD(32, sent); if (sent) MDA(33); MDA(34);   /* NOTES 51: tiles, non-empty batches, batches.
                                                      * WAS [16]/[17]/[18] -- collided with the
                     ...
                     * Layout: [n] then n x [(plane<<7)|strip, value]. */
                    {
                        ...
                        if (md_pending && o + MD_REC_W + 1 <= sc + 688) {   /* idle: no scan */
                            ...
                            *na = (uint16_t)n;
                            ...
                            DIAG[57] += (uint32_t)n;
#ifdef MD_ALLOC_WHY
                            MDA_ADD(37, n);              /* NOTES 51: art riding a chunk's tail.
                                                      * WAS [21] -- collided with mdp_claim_pen. */
#endif
                        }
#endif
#else
