...
---------------------------------------------------------------------
## 274. THE COLOUR-LINE BAKE ALREADY SHIPS. THE REAL GAP IS ONE SHARED AREA MISSING FROM ROUNDS 2 AND 4, AND THE TABLES ARE NEARLY FULL (2026-09-13 22:40)

Before wiring anything from 272's new baker I checked what the line
already does, and the answer retires most of the card.

**`sh_src/pal_rounds_md.h` IS a per-round colour-to-LINE bake and it is
on the shipping line.** `MDROUND_N 5`, generated by
`tools/bake_tilecram.py --emit-mds` from each round's own gated live
palette dumps plus the worst-case viewport over all 64 scroll
positions, both planes; indexed by the GAME'S round, not by a
palette-detected scene. `mdr_line_c[round][48]` is three CRAM lines of
pinned colours, `mdr_s_line[round][128]` is set -> line,
`mdr_s_map[round][1024]` is (set, pixel) -> pen. The runtime consumes it
at m_main.c 2415-2429 and REFUSES a set the round's table does not hold
(MDS_REFUSE). MDSTATIC=1 MDROUND=1 are both in the line's flags.

So NOTES 62's "MDSTATIC pins SETS to slots; this pins COLOURS to lines,
which is the step it stopped short of" is not true of this line -- that
step was taken in LOOP29 192. The idea is right and it is already built.

**What IS missing, measured against the decompile thread's 20-scene set
lists (docs/audit/mdpen_scene_sets.txt):**

    round   sets in table   sets the thread sees   missing
...
**Rounds 2 and 4 are missing the ENTIRE sets 22-36 group** -- which is
exactly the area 272 identified as SHARED between round 2 and round 4
(the same section of level art reached in both). Rounds 0, 1 and 3 are
fully covered. So `bake_tilecram.py`'s live dumps sampled one area per
round and never reached the second, and every tile of sets 22-36 in
rounds 2 and 4 falls through the static table to the dynamic path,
where it can evict. That is LOOP29 231's black-set signature with a
named cause and a named location.

**And it is a capacity problem, not an oversight to patch.** The
shipped tables are nearly full:

    round 0: 30 sets -> lines [14, 14, 15]   2 pens spare
    round 1: 15 sets -> lines [15, 14, 15]   1 pen  spare
    round 2: 16 sets -> lines [14, 15, 12]   4 pens spare
    round 3: 14 sets -> lines [15,  9, 11]  10 pens spare
    round 4: 19 sets -> lines [14, 15, 11]   5 pens spare

Thirteen more sets do not go into 4 or 5 spare pens. 272's conservative
bake of round 2's FULL union (24 sets) fits at 39 colours -> [15,13,15],
exactly full, and round 4's full union (27 sets) does NOT fit at all
(46 against 45, still unpackable at 43 with pixel 0 excluded). So:

  - **round 2 can be rebuilt as one table** covering both areas, with
    zero pens spare;
  - **round 4 must be split in two**, and needs the discriminator bit
    NOTES 66 asked the decompile thread for.

**The two data sources disagree and must be reconciled before either
rebuild.** The shipped tables come from live gated dumps over every
scroll position; the thread's lists come from the arcade's visible
windows in the demos. Round 2 shows the disagreement plainly: shipped
uses 41 pens for 16 sets, the thread's conservative union needs 39 for
24. Neither is a superset of the other, so the rebuild input is the
UNION of both and the packing has to be re-run on that union -- not on
either source alone.

`tools/bake_mdlines.py` (272) duplicates shipped machinery and is NOT a
second baker. It stays as the ANALYSIS tool that produced this: it
