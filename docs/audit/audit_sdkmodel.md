...
- **The port**: `/Users/mikeholzinger/src/sega16-2-32x/` — NOTES.md, docs/log/DEVNOTES.md, sh_src/, md_src/.
...
d32xr: 68K asserts CMD INT for pad delivery, DREQ kickoff, everything (`src-md/crt0.s:3143, 3175`; SH-2 side `crt0.s:598-689`, `marshw.c:1055-1069`). Port: LOOP11 built CMDPROBE/CMDINT (md_main.c:1005-1006 cites d32xr src-md/crt0.s:3143), measured it three rounds, and retired it — "NEVER SHIP. The prize was always smaller than the price" (docs/log/LOOP11.md:956, honored by LOOP23/24 per git 45c3b22). Legitimately measured-out for the *window pickup* use, but it means the port has **no asynchronous 68K→SH-2 signal at all**; everything rides polled COMM/SDRAM mailboxes with hand-tuned poll cadences (e.g. `s_main.c:27-67` preempt-blit mailbox, throttled idle meters `s_main.c:97-104`).
